...
foreach($WebApplication in $WebApplications)
{
    ...
    {   
        if($Site -ne $null)
        {
            if( -not($Site.Url.StartsWith("http://fedvshpapp01.ftbco.ftn.com/personal/","CurrentCultureIgnoreCase")) -and -not($Site.Url.StartsWith("https://test-mysites","CurrentCultureIgnoreCase")) -and -not($Site.Url.StartsWith("https://mysites","CurrentCultureIgnoreCase")) )
            {
